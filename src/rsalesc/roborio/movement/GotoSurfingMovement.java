...
import rsalesc.roborio.utils.stats.Segmentation;
...
public class GotoSurfingMovement extends Movement {
    ...
    private List<rsalesc.roborio.utils.geo.Point>           _predicted;
    ...
    public void onHitByBullet(HitByBulletEvent e) {
        ...
        rsalesc.roborio.utils.geo.Point hitPoint = new rsalesc.roborio.utils.geo.Point(e.getBullet().getX(), e.getBullet().getY());
    ...
    public void onScan(ScannedRobotEvent e) {
        ...
        if(R.nearOrBetween(-Physics.MAX_POWER, energyDelta, -Physics.MIN_POWER)) {
            ...
            onFire(targetLog.getLatest(), -energyDelta);
        } else {
            // flattening
            double power = 1.0;
    ...
    public void doMovement() {
        ...
        rsalesc.roborio.utils.geo.Point currentLocation = myLog.getLatest().getPoint();
        ...
        _predicted = new ArrayList<rsalesc.roborio.utils.geo.Point>();
...
//        st.setSmoother(new GaussianSmoothing(bandwidth));
    ...
    private double getGf(EnemyWave wave, rsalesc.roborio.utils.geo.Point hitPoint) {
        return getGf(wave, Physics.absoluteBearing(wave.getSource(), hitPoint));
    }

    private double getUnconstrainedGf(EnemyWave wave, rsalesc.roborio.utils.geo.Point hitPoint) {
    ...
    private DangerPoint getDanger(EnemyFireWave wave, int direction, GuessFactorStats st) {
        ...
        rsalesc.roborio.utils.geo.Point res = null;
        rsalesc.roborio.utils.geo.Point last = null;
        rsalesc.roborio.utils.geo.Point back = genPoints.get(genPoints.size() - 1);
        ...
        for(PredictedPoint predicted : genPoints) {
            if(last == null || predicted == back || last.distance(predicted) > 20) {
                ...
                rsalesc.roborio.utils.geo.Point impactPoint = data.getMidwayImpactPoint();
...
//                double value = st.getValue(gfRange.getCenter());
    ...
    private double getSteepness(rsalesc.roborio.utils.geo.Point dest, rsalesc.roborio.utils.geo.Point enemy) {
        rsalesc.roborio.utils.geo.Point my = getRobot().getPoint();
    ...
    private void goTo(rsalesc.roborio.utils.geo.Point dest) {
        if(_surfedWave != null && _gotoPoint != null) {
            getRobot().moveWithBackAsFront(dest, getRobot().getPoint().distance(dest));
        }
    }

    private void log(EnemyWave wave, rsalesc.roborio.utils.geo.Point hitPoint) {
    ...
    public void onPaint(Graphics2D graphics) {
        ...
        if(_predicted != null) {
            for(rsalesc.roborio.utils.geo.Point point : _predicted) {
        ...
        for(Wave cur : waves) {
            ...
            for(int i = 0; i < WAVE_DIVISIONS; i++) {
                ...
                rsalesc.roborio.utils.geo.Point hitPoint = wave.getSource().project(angle, wave.getDistanceTraveled(getTime()));
