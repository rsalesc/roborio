...
public class RedManhattanTree<T> {
    ...
    public ArrayList<rsalesc.roborio.structures.KdTree.Entry<T>> kNN(double[] query, int K, double alpha) {
        if(K == 0)
            return new ArrayList<>();

        List<KdTree.Entry<T>> list = t.nearestNeighbor(query, K, false);

        ArrayList<rsalesc.roborio.structures.KdTree.Entry<T>> res = new ArrayList<>();
        for(KdTree.Entry<T> entry : list) {
            res.add(new rsalesc.roborio.structures.KdTree.Entry<T>(entry.distance, entry.value));
        }
